...
@interface LookImageViewController ()<UITableViewDataSource,UITableViewDelegate>
...
- (NSMutableArray *)dataArr{
	if (!_dataArr) {
        ...
        [_dataArr addObjectsFromArray:[ZHSaveDataToFMDB selectDataWithIdentity:@"LookImageCellModel"]];
	}
	return _dataArr;
}
...
- (void)addProjectAction{
    ...
    [ZHAlertAction alertWithTitle:@"添加新的工程" withMsg:Msg addToViewController:self withCancleBlock:nil withOkBlock:^{
        ...
        NSArray *fileArr=[ZHFileManager subPathFileArrInDirector:path hasPathExtension:@[@"png", @"jpg", @"jpeg", @"gif", @"bmp", @"pdf"]];
        
        if(fileArr.count==0){
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.15 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [ZHAlertAction alertWithMsg:@"工程路径不存在图片资源文件!请重新填写!" addToViewController:self ActionSheet:NO];
            });
            return;
        }
        ...
        for (LookImageCellModel *model in self.dataArr) {
        ...
        if (needAdd==YES) {
            LookImageCellModel *StroyBoardCreatePropertyModel=[LookImageCellModel new];
        ...
        if (fileArr.count>0) {
            [ZHSaveDataToFMDB insertDataWithData:self.dataArr WithIdentity:@"LookImageCellModel"];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
	...
	if ([modelObjct isKindOfClass:[LookImageCellModel class]]){
		LookImageTableViewCell *StroyBoardCreatePropertyCell=[tableView dequeueReusableCellWithIdentifier:@"LookImageTableViewCell"];
		LookImageCellModel *model=modelObjct;
