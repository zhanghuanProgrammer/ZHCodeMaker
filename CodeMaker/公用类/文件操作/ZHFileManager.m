...
#pragma mark 删除文件(夹)操作
/**删除文件夹,如果它存在*/
+ (void)deleteDirectorIfExsit:(NSString *)DirectorPath{
    if([self fileExistsAtPath:DirectorPath])
        [ZHFileManager removeItemAtPath:DirectorPath];
}
/**清空文件夹,如果它存在*/
+ (void)clearDirectorIfExsit:(NSString *)DirectorPath{
    if([self fileExistsAtPath:DirectorPath]){
        [ZHFileManager removeItemAtPath:DirectorPath];
        [ZHFileManager createDirectoryAtPath:DirectorPath];
    }
}
/**删除批量文件*/
+ (void)deleteFiles:(NSArray *)files{
    for (NSString *file in files) {
        if([self fileExistsAtPath:file])
            [ZHFileManager removeItemAtPath:file];
    }
}
/**删除子路径 文件(带特定后缀) */
+ (void)deleteSubPathFileArrInDirector:(NSString *)DirectorPath hasPathExtension:(NSArray *)pathExtension{
    NSArray *files=[self subPathFileArrInDirector:DirectorPath hasPathExtension:pathExtension];
    [self deleteFiles:files];
}
/**删除子路径 文件(文件名包含指定字符串) */
+ (void)deleteSubPathFileArrInDirector:(NSString *)DirectorPath fileNameContain:(NSArray *)contents{
    NSArray *files=[self subPathFileArrInDirector:DirectorPath fileNameContain:contents];
    [self deleteFiles:files];
}
...
+ (NSArray *)contentsDirectorArrInDirector:(NSString *)DirectorPath{
    if (![[NSFileManager defaultManager]fileExistsAtPath:DirectorPath]) {
        return nil;
    }
    NSArray *arrTemp=[[NSFileManager defaultManager]contentsOfDirectoryAtPath:DirectorPath error:nil];
    NSMutableArray *pathDirectorArr=[NSMutableArray array];
    for (NSString *str in arrTemp) {
        if (str.length>0) {
            if ([self getFileType:[DirectorPath stringByAppendingPathComponent:str]]==FileTypeDirectory) {
