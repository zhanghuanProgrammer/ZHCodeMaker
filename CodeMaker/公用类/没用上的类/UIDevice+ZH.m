...
static uint64_t net_counter_get_by_type(net_interface_counter *counter, NetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & NetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & NetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
