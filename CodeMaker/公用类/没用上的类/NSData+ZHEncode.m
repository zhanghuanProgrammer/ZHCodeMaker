#import "NSData+ZHEncode.h"
#include <CommonCrypto/CommonCrypto.h>
#include <zlib.h>

@implementation NSData (ZHEncode)

- (NSString *)md2String {
    unsigned char result[CC_MD2_DIGEST_LENGTH];
    CC_MD2(self.bytes, (CC_LONG)self.length, result);//重要函数
...
- (NSString *)md4String {
    ...
    CC_MD4(self.bytes, (CC_LONG)self.length, result);//重要函数
    return [NSString stringWithFormat:
            @"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            result[0], result[1], result[2], result[3],
            result[4], result[5], result[6], result[7],
...
- (NSString *)md5String {
    ...
    CC_MD5(self.bytes, (CC_LONG)self.length, result);//重要函数
    return [NSString stringWithFormat:
            @"%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            result[0], result[1], result[2], result[3],
            result[4], result[5], result[6], result[7],
...
- (NSString *)sha1String {
    ...
    CC_SHA1(self.bytes, (CC_LONG)self.length, result);//重要函数
...
- (NSString *)sha224String {
    ...
    CC_SHA224(self.bytes, (CC_LONG)self.length, result);//重要函数
...
- (NSString *)sha256String {
    ...
    CC_SHA256(self.bytes, (CC_LONG)self.length, result);//重要函数
...
- (NSString *)sha384String {
    ...
    CC_SHA384(self.bytes, (CC_LONG)self.length, result);//重要函数
...
- (NSString *)sha512String {
    ...
    CC_SHA512(self.bytes, (CC_LONG)self.length, result);//重要函数
