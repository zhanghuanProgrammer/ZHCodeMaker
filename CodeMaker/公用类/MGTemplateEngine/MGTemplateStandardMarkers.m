...
#define FOR_TYPE_ENUMERATOR				@"in"	
#define FOR_TYPE_RANGE					@"to"	
			...
			if (valid) {
				*blockStarted = YES;
				...
				if (blockInfo) {
					...
					parentLoop = (NSDictionary *)[engine resolveVariable:FOR_LOOP_VARS]; 
					if (parentLoop) {
			...
			*blockStarted = YES;
			...
			if ([args count] == 1) {
				...
				if ([op isEqualToString:@"and"] || [op isEqualToString:@"&&"]) {
                    ...
                    if ([secondArg isEqualToString:@"nil"]) {
                        NSObject *firstVal = [engine resolveVariable:firstArg];
                        argTrue = firstVal != nil;
                    }else{
                        argTrue = (num1 != num2);
                    }
	...
	if (arg && [arg length] > 0) {
		if ([[arg substringToIndex:1] isEqualToString:@"0"] || [arg integerValue] != 0) {
			...
			if (argObj && [argObj respondsToSelector:@selector(integerValue)] &&
				[self argIsNumeric:argStr integerValue:&value checkVariables:NO]) { 
				numeric = YES;
			}
		}
	}
