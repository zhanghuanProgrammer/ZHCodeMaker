...
+ (void)saveDataHasTempFileWithData:(id)data WithIdentity:(NSString *)identity{
    ...
    //如果数据是字典
        ...
        NSString *RandFileName=[FilePath stringByAppendingString:[NSString stringWithFormat:@"%@.txt",[ZHNSString getRandomStringWithLenth:26]]];
...
+ (FMDatabase *)getDataBase{
    if (dataBase==nil) {
        ...
        NSLog(@"%@",[NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@",DataBaseName]);
        if (![dataBase open]) {
            NSLog(@"数据库创建失败");
        }
...
+ (void)testTypeOfDataWithIdentity:(NSString *)identity{
    ...
    if([obj isKindOfClass:[NSArray class]]){
        NSLog(@"缓存 %@ 返回的是数组(NSArray)",identity);
    }else if([obj isKindOfClass:[NSDictionary class]]){
        NSLog(@"缓存 %@ 返回的是字典(NSDictionary)",identity);
    }else{
        NSLog(@"缓存 %@ 返回的数据有误,请确保存入进去的是数组,字典,URL(JSON),JSON字符串",identity);
    }
}
//测试获取的目标数据是哪张类型
