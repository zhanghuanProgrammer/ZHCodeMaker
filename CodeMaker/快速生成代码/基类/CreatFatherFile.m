...
- (NSString *)getInfoFromDic:(NSArray *)arr{
    ...
    for (NSInteger i=0; i<arr.count; i++) {
        NSString *text=arr[i];
        NSString *key=text,*value=@"\"<#请填写#>\"";
        if ([ZHNSString contain:@"--:--" inText:text]) {
        ...
        if (i!=arr.count-1) {
            [self insertValueAndNewlines:@[[NSString stringWithFormat:@"\"%@\":%@,",key,value]] ToStrM:strM];
        }else{
            [self insertValueAndNewlines:@[[NSString stringWithFormat:@"\"%@\":%@",key,value]] ToStrM:strM];
        }
    }
    [strM appendString:@"}"];
    [strM setString:[[ZHWordWrap new]wordWrapText:strM]];
    return strM;
}
                ...
                if (subDic.count>0) {
                    ...
                    if ([subDicKey isEqualToString:@"0"]) {
                        ...
                        [text appendFormat:@"<subviews>\n\
                         ...
                         </subviews>\n",imageViewID,LabelID,TextFiledID];
                        ...
                        for (NSString *subCell in arrCells) {
                            ...
                            [text appendFormat:@"<tableViewCell clipsSubviews=\"YES\" contentMode=\"scaleToFill\" selectionStyle=\"default\" indentationWidth=\"10\" reuseIdentifier=\"%@TableViewCell\" rowHeight=\"124\" id=\"%@\" customClass=\"%@TableViewCell\">\n\
                             ...
                             </tableViewCell>\n",subCell,idText3,subCell,idText3,idText4,imageViewID_new,LabelID_new,TextFiledID_new];
                        }
